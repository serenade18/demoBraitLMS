...
class VerificationController extends Controller
{
        ...
        if (empty($authUser)) {
            ...
            if (!empty($authUser)) {
                if (\Auth::loginUsingId($authUser->id)) {
                    // Handle referral code and bonuses
                    if (!empty($referralCode)) {
                        Affiliate::storeReferral($authUser, $referralCode);
                    }

                     // Check if the user is still "pending", and update to "active"
                if ($authUser->status == User::$pending) {
                    ...
                    // After login, redirect to the intended URL
                    return $this->redirectAfterConfirmation($request);
                }
            }
    ...
    protected function redirectAfterConfirmation(Request $request)
    {
    // Get the redirect URL passed in the form or default to '/panel'
    $redirectTo = session()->pull('intended_url', '/panel');
    
    // Perform the redirect to the URL
