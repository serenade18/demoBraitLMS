...
class WebinarCertificateController extends Controller
{
    ...
    public function showCourseCertificate($certificateId)
    {
        ...
        if (!empty($certificate)) {
            ...
            // Check if certificate fee is paid
            // if (!$certificate->certificate_fee_paid) {
            //     return redirect()->route('certificates.fee', ['certificateId' => $certificateId])
            //         ->with('error', 'You need to pay the certificate fee to download this certificate.');
            // }
    ...
    public function showBundleCertificate($certificateId)
    {
        ...
        if (!empty($certificate)) {
            ...
            // // Check if certificate fee is paid
            // if (!$certificate->certificate_fee_paid) {
            //     return redirect()->route('certificates.payment', ['certificateId' => $certificateId])
            //         ->with('error', 'You need to pay the certificate fee to download this certificate.');
            // }
            $makeCertificate = new MakeCertificate();
