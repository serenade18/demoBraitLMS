...
@section('content')
    ...
    <section
        ...
        <div class="row">
            <div class="course-content-sidebar col-12 col-lg-4 mt-25 mt-lg-0 order-2 order-lg-12">

                <div class="rounded-lg shadow-sm d-none d-lg-block">
                            ...
                            <div class="mt-20 d-flex flex-column">
                                ...
                                @elseif(!empty($course->price) and $course->price > 0)
                                    ...
                                    @if ($canSale and !empty($course->points))
                                        <button type="button"
                                            class="btn btn-outline-danger mt-20  {{ $canSale ? 'js-course-add-to-cart-btn' : $course->cantSaleStatus($hasBought) . ' disabled ' }}">
                                            @if (!$canSale)
                                                @if ($course->checkCapacityReached())
                                                    {{ trans('update.capacity_reached') }}
                                                @else
                                    ...
                                    @if ($canSale && !empty(getFeaturesSettings('direct_classes_payment_button_status')))
                                        @auth
                                ...
                                @else
                                    <a href="#"
                                        ...
                                        onclick="event.preventDefault(); handleEnrollClick({{ auth()->check() ? 'true' : 'false' }}, {{ $canSale ? 'true' : 'false' }}, '{{ url('/course/' . $course->slug . '/free') }}');">
                                        @if (!$canSale)
                                            @if ($course->checkCapacityReached())
                                                {{ trans('update.capacity_reached') }}
                                            @else
                        ...
                        <div id="signupForm" style="display: none;">
                            ...
                            <form method="post" action="/register" class="mt-35">
                                ...
                                <input type="hidden" name="redirect_to"
                                    value="{{ session('intended_url', url()->current()) }}">
                ...
                @if ($course->teacher->offline)
                    <div class="rounded-lg shadow-sm mt-35 d-flex">
                        ...
                        <div class="p-15">
                            ...
                            <p class="font-14 font-weight-500 text-gray mt-15">{{ $course->teacher->offline_message }}
                            </p>
                        </div>
                    </div>
                @endif

                <div class="rounded-lg shadow-sm mt-35 px-25 py-20 order-3 order-lg-2 ">
            ...
            <div class="col-12 col-lg-8">
                <div class="course-content-body user-select-none container">


                    <div class="course-body-on-cover text-white order-1 order-lg-1 mb-40">
                    ...
                    <div class="rounded-lg shadow-sm d-block  d-lg-none">
                        <div class="course-img {{ $course->video_demo ? 'has-video' : '' }}">
                        ...
                        <div class="px-20 pb-30">
                                    ...
                                    @foreach ($course->tickets as $ticket)
                                        <div class="form-check mt-20">
                                            <input class="form-check-input"
                                                @if (!$ticket->isValid()) disabled @endif type="radio"
                                                data-discount-price="{{ handleCoursePagePrice($ticket->getPriceWithDiscount($course->price, !empty($activeSpecialOffer) ? $activeSpecialOffer : null))['price'] }}"
                                                value="{{ $ticket->isValid() ? $ticket->id : '' }}" name="ticket_id"
                                                id="courseOff{{ $ticket->id }}">
                                            <label class="form-check-label d-flex flex-column cursor-pointer"
                                            ...
                                            <div class="text-center">
                                                ...
                                                @if (!empty($priceWithDiscount['tax']))
                                                    <span class="d-block font-14 text-gray">+
                                                        {{ $priceWithDiscount['tax'] }} {{ trans('cart.tax') }}</span>
                                                @endif
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div class="mt-20 d-flex flex-column">
                                    ...
                                    @else
                                        <a href="javascript:void(0);"
                                            class="btn btn-primary {{ !$canSale ? 'disabled ' . $course->cantSaleStatus($hasBought) : '' }}"
                                            onclick="handleAuthClick({{ auth()->check() ? 'true' : 'false' }}, '{{ $canSale ? route('webinar.free', ['slug' => $course->slug]) : '#' }}')">
                                            @if (!$canSale)
                                                @if ($course->checkCapacityReached())
                                                    {{ trans('update.capacity_reached') }}
                                                @else
                            ...
                            <div class="mt-40 p-10 rounded-sm border row align-items-center favorites-share-box">
                                ...
                                <div class="col">
                                    <a href="/favorites/{{ $course->slug }}/toggle" id="favoriteToggle"
                                        ...
                                        <i data-feather="heart"
                                            class="{{ !empty($isFavorite) ? 'favorite-active' : '' }}" width="20"
                                            height="20"></i>
                                        <span class="font-12">{{ trans('panel.favorite') }}</span>
                                    </a>
                                </div>
                                ...
                                <div class="col">
                                    <a href="#"
                                        class="js-share-course d-flex flex-column align-items-center text-gray">
                                        <i data-feather="share-2" width="20" height="20"></i>
                                        <span class="font-12">{{ trans('public.share') }}</span>
                                    </a>
                                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="loginModalLabel">{{ trans('auth.login') }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="/login" class="mt-35">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="hidden" name="redirect_to" id="intendedUrlInput" value="{{ url()->current() }}">
                    ...
                    <div class="mt-20 text-center">
                        ...
                        <a href="{{ route('register', ['redirect_to' => url()->current()]) }}"
...
@push('scripts_bottom')
    <script>
        function handleEnrollClick(isAuthenticated, canSale, enrollUrl) {
            if (isAuthenticated) {
                if (canSale) {
                    window.location.href = enrollUrl; // Redirect to the course page if the user can enroll
                }
            } else {
                // Store the intended URL in the modal’s hidden input field
                document.getElementById('intendedUrlInput').value = enrollUrl;
        ...
        function handleAuthClick(isAuthenticated, enrollUrl) {
            if (isAuthenticated) {
                // If the user is authenticated, redirect to the course's URL
                window.location.href = enrollUrl;
            } else {
                // If not authenticated, store the current URL and show the login modal
                sessionStorage.setItem('redirect_to', window.location.href); // Save current URL in sessionStorage
