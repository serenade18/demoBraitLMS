...
@push('scripts_bottom')
    ...
    <script>
document.getElementById('toggleOutdateCourse').addEventListener('click', function () {
    const webinarId = this.getAttribute('data-webinar-id');
    const currentOutdated = this.getAttribute('data-outdated') == 1;

    const actionUrl = currentOutdated 
        ? `/webinars/${webinarId}/unoutdate` 
        : `/webinars/${webinarId}/outdate`;
    ...
    fetch(actionUrl, {
        ...
        .then(data => {
            if (data.success) {
                this.setAttribute('data-outdated', currentOutdated ? 0 : 1);
                this.classList.toggle('btn-danger');
                this.classList.toggle('btn-success');
                this.textContent = currentOutdated ? 'Outdate' : 'Unoutdate';
                alert(data.message);
            } else {
